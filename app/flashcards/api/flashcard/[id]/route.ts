...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to update flashcard: ' + error.message }, { status: 500 });
    }
    ...
    return NextResponse.json(
      { error: 'Server error: ' + (error instanceof Error ? err.message : 'Unknown error') },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to delete flashcard: ' + error.message }, { status: 500 });
    }
